...
- **A stricter stop on unknown keeps the rule's cap.** For a `REDUCE_TO(X)` rule whose predicate is `UNKNOWN` and whose `on_unknown` is `REQUIRE_USER` or `BLOCK`, both apply: the capability reduction to `X` **and** the stop outcome — so choosing the stricter unknown behaviour can never record a higher `effective_capability` than a known match. Weaker `on_unknown` choices the schema permits on non-BLOCK rules (`NO_EFFECT`, a milder `REDUCE_TO`) remain the user's declared policy behaviour.
