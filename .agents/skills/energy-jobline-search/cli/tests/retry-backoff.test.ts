...
// 500ms -> 8s backoff schedule (or the 10s crawl delay between requests).
//
...
describe("htmlFetch retry/backoff", () => {
  test("retries a 429 and succeeds on the next attempt", async () => {
    ...
    const html = await htmlFetch("https://www.energyjobline.com/x")
  ...
  test("gives up after the initial attempt plus six retries on persistent 5xx", async () => {
    ...
    await expect(htmlFetch("https://www.energyjobline.com/x")).rejects.toThrow(/500/)
